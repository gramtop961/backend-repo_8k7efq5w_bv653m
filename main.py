...
app = FastAPI(title="NGO Site API")
...
@app.get("/")
def read_root():
    return {"message": "NGO API running"}

# Schemas for incoming create requests (reuse Pydantic from schemas)
from schemas import Galleryitem, News, Contactmessage
...
def list_gallery(limit: int = 50):
    try:
        items = get_documents("galleryitem", {}, limit)
        return [serialize_doc(x) for x in items]
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/gallery", status_code=201)
def upload_gallery(item: Galleryitem):
    try:
        inserted_id = create_document("galleryitem", item)
...
def create_news(item: News):
    try:
        inserted_id = create_document("news", item)
        return {"id": inserted_id}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
